import { Angle, Point, TVectorValues, Vector } from '@abstracts';
import { TSegment } from '@figures';
...
it('should have magnitude 0 for a zero vector', () => {
  const vector = new Vector([0, 0]);

  expect(vector.magnitude).toBe(0);
});

it('should have dot product 0 with a zero vector', () => {
  const vector = new Vector([3, 4]);
  const zero = new Vector([0, 0]);

  expect(vector.dotProduct(zero)).toBe(0);
});

it('should remain a zero vector after scaling by 0', () => {
  const vector = new Vector([5, 8]);
